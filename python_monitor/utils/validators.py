...
def validate_slack_token(token):
    ...
    parts = token.split('-')
    if len(parts) < 4:
        raise ValidationError("Invalid Slack token format. Expected format: xoxb-{team_id}-{bot_id}-{secret}")
...
def validate_slack_channel(channel):
    ...
    if not re.match(r'^[a-zA-Z0-9_-]+$', channel_name):
        raise ValidationError("Invalid Slack channel format. Must contain only letters, numbers, hyphens, or underscores")
...
def validate_config_section(config_dict, section_name, required_fields=None, optional_fields=None):
    ...
    for field in required_fields:
        if field not in config_dict:
            raise ValidationError(f"Missing required field '{field}' in {section_name} configuration")
