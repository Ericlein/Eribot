...
class ConfigLoader:
        ...
        try:
            ...
            logging_config = LoggingConfig(
                ...
                console_output=os.getenv(
                    "LOG_CONSOLE", str(logging_yaml.get("console_output", True))
                ).lower()
                in ("true", "1", "yes"),
            )
            ...
            print(
                f"Config: CPU={monitoring_config.cpu_threshold}%, "
    ...
    def validate(self) -> bool:
        ...
        if not (1 <= self._config.monitoring.cpu_threshold <= 100):
            errors.append(
                f"CPU threshold must be between 1 and 100, got {self._config.monitoring.cpu_threshold}"
            )

        if not (1 <= self._config.monitoring.memory_threshold <= 100):
            errors.append(
                f"Memory threshold must be between 1 and 100, got {self._config.monitoring.memory_threshold}"
            )

        if not (1 <= self._config.monitoring.disk_threshold <= 100):
            errors.append(
                f"Disk threshold must be between 1 and 100, got {self._config.monitoring.disk_threshold}"
            )

        # Validate check interval (5 seconds to 1 hour)
        if not (5 <= self._config.monitoring.check_interval <= 3600):
            errors.append(
                f"Check interval must be between 5 and 3600 seconds, got {self._config.monitoring.check_interval}"
            )
        ...
        if not self._config.remediator.url.startswith(("http://", "https://")):
            errors.append(
                f"Remediator URL must start with http:// or https://, got {self._config.remediator.url}"
            )
        ...
        if self._config.logging.level.upper() not in valid_levels:
            errors.append(
                f"Log level must be one of {valid_levels}, got {self._config.logging.level}"
            )
