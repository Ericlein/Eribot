...
class TestConfigModels:
    ...
    def test_slack_config_defaults(self):
        ...
        config = SlackConfig(channel="#test", token="xoxb-test-token")
        assert config.channel == "#test"
        assert config.token == "xoxb-test-token"
    ...
    def test_remediator_config_defaults(self):
        ...
        config = RemediatorConfig(url="http://localhost:5001")
        assert config.url == "http://localhost:5001"
        assert config.timeout == 30
        assert config.retry_attempts == 3
    ...
    def test_logging_config_creation(self):
        ...
        config = LoggingConfig(
            level="DEBUG", max_file_size="20MB", backup_count=10, console_output=False
        )
    ...
    def test_app_config_creation(self):
        ...
        config = AppConfig(
            monitoring=monitoring, slack=slack, remediator=remediator, logging=logging
        )
